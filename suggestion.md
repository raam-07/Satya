...
### B. Zero-Hardcoded Name Disambiguation
In political news, politicians are often referred to by generic surnames ("Modi", "Shah", "Yadav", "Gandhi"). Simple substring scans trigger false positives. 

To eliminate hardcoded entity maps in your Python script, implement a **Dynamic Metadata Heuristic** that automatically extracts contextual anchor terms directly from each politician's fields in `entities.json` (such as `name`, `party`, `state`, `constituency`, `role`, and `ministry`) at runtime.

* **Context Window Verification**: When an ambiguous surname is matched in raw text, scan a 200-character boundary around it to verify the presence of at least one dynamically-derived contextual anchor.
* **Preserving Explicit Data**: Always bypass this context filter for multi-word matches (like `"Narendra Modi"`) or explicit tags generated by the primary AI classifier, ensuring maximum safety against data loss.

```python
# 1. On-the-fly keyword extractor from entities.json properties
def generate_context_keywords(minister_entity):
    keywords = set()
    
    # Extract parts of name, constituency, party, and state
    ...
    # Extract role descriptions and generate abbreviations (e.g., "pm", "cm")
